...
ll operator+(const ll& lval, const ll& rval) {
    ...
    lvec.size() > rvec.size() ? rvec.resize(lvec.size(), 0) : lvec.resize(rvec.size(), 0);//Дополнение нулями в случае не совпадения длин
    ll result(0, std::vector<long long>(lvec.size() + 1, 0));
    if (lval.abs() < rval.abs()) std::swap(lvec, rvec);
    bool sf = (rval.sign != lval.sign); //Флаг знака
